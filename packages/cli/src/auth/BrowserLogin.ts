...
export async function browserLogin(): Promise<{
  ...
  try {
    ...
    // Successful login lands on any authenticated route under input.riseup.co.il
    // (e.g. /home, /sr, /ob, /transactions, etc.).
    await page.waitForURL(
      (url) => {
        const href = url.toString();
        if (!href.startsWith("https://input.riseup.co.il/")) return false;
        if (href.includes("/login")) return false;
        if (href.includes("login-enter-code")) return false;
        if (href.includes("/signup")) return false;
        if (href.includes("/web/signup")) return false;
        return true;
      },
      { timeout: 5 * 60 * 1000 },
    );

    // Give the app a moment to persist cookies (set via JS from query params).
    ...
    const authCookiePresent = playwrightCookies.some((c) => c.name === "__Host-auth-token");
    if (!authCookiePresent) {
      throw new Error("Auth token cookie not found — login may not have completed.");
    }
