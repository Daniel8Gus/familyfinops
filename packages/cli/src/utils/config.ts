...
/** Full path to the session file for the given profile (or default). */
export function getSessionPath(profile?: string): string {
  if (profile) {
    return join(getConfigDir(), `session-${profile}.json`);
  }
